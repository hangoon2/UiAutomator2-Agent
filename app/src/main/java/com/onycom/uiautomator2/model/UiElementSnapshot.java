...
public class UiElementSnapshot extends UiElement<AccessibilityNodeInfo, UiElementSnapshot> {
    ...
    private UiElementSnapshot(AccessibilityNodeInfo node, int index, int maxDepth) {
        ...
        setAttribute(attributes, Attribute.PACKAGE, StringHelpers.charSequenceToNullableString(node.getPackageName()));
        setAttribute(attributes, Attribute.CLASS, StringHelpers.charSequenceToNullableString(node.getClassName()));
        ...
        setAttribute(attributes, Attribute.CONTENT_DESC, StringHelpers.charSequenceToNullableString(node.getContentDescription()));
    ...
    private void addToastMsgToRoot(CharSequence tokenMSG) {
        ...
        ReflectionUtils.setField("mSealed", true, node);
    ...
    private List<UiElementSnapshot> buildChildren(AccessibilityNodeInfo node) {
        ...
        boolean areInvisibleElementsAllowed = AppiumUIA2Driver
                .getInstance()
                .getSessionOrThrow()
                .getCapability(Settings.ALLOW_INVISIBLE_ELEMENTS.toString(), false);
