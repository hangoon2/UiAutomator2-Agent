...
import com.onycom.uiautomator2.model.api.ElementModel;
import com.onycom.uiautomator2.model.api.touch.w3c.W3CGestureModel;
import com.onycom.uiautomator2.model.api.touch.w3c.W3CItemModel;
...
public class ActionsTokenizer {
    ...
    private static long alignDuration(long ms) {
        ...
        if (ms < ActionsConstants.EVENT_INJECTION_DELAY_MS) {
            return ActionsConstants.EVENT_INJECTION_DELAY_MS;
        }
        final long modResult = ms % ActionsConstants.EVENT_INJECTION_DELAY_MS;
        return modResult == 0 ? ms : ms + ActionsConstants.EVENT_INJECTION_DELAY_MS - modResult;
    }
    ...
    private static int extractButton(final W3CGestureModel gesture, final int toolType) {
        ...
        int button = gesture.button == null ? ActionsConstants.MOUSE_BUTTON_LEFT : gesture.button;
        // W3C button codes are different from Android constants. Converting...
        switch (button) {
            case ActionsConstants.MOUSE_BUTTON_LEFT:
            ...
            case ActionsConstants.MOUSE_BUTTON_MIDDLE:
                return MotionEvent.BUTTON_TERTIARY;
            case ActionsConstants.MOUSE_BUTTON_RIGHT:
                if (toolType == MotionEvent.TOOL_TYPE_STYLUS && Build.VERSION.SDK_INT >= 23) {
    ...
    private static long extractDuration(final W3CItemModel item, final W3CGestureModel gesture) {
        if (gesture.duration == null) {
            throw new ActionsParseException(String.format(
                    ...
                    ActionsConstants.ACTION_ITEM_DURATION_KEY, gesture, item.id));
        }
    ...
    private static int actionToToolType(final W3CItemModel item) {
        if (item.parameters != null && item.parameters.pointerType != null) {
            switch (item.parameters.pointerType) {
                case ActionsConstants.POINTER_TYPE_MOUSE:
                    return MotionEvent.TOOL_TYPE_MOUSE;
                case ActionsConstants.POINTER_TYPE_PEN:
                    return MotionEvent.TOOL_TYPE_STYLUS;
                case ActionsConstants.POINTER_TYPE_TOUCH:
    ...
    private static MotionEvent.PointerCoords extractCoordinates(final String actionId,
        ...
        if (!ActionsConstants.ACTION_ITEM_TYPE_POINTER_MOVE.equals(gesture.type)) {
            if (itemIdx > 0) {
        ...
        Object origin = gesture.origin == null ? ActionsConstants.ACTION_ITEM_ORIGIN_VIEWPORT : gesture.origin;
        final MotionEvent.PointerCoords result = new MotionEvent.PointerCoords();
        ...
        if (origin instanceof String) {
            if (origin.equals(ActionsConstants.ACTION_ITEM_ORIGIN_VIEWPORT)) {
                ...
                result.y = gesture.y.floatValue();
                return result;
            } else if (origin.equals(ActionsConstants.ACTION_ITEM_ORIGIN_POINTER)) {
                if (itemIdx > 0) {
    ...
    private void applyEmptyActionToEventsMapping(final W3CItemModel item) {
        ...
        for (final W3CGestureModel gesture : gestures) {
            if (!ActionsConstants.ACTION_ITEM_TYPE_PAUSE.equals(gesture.type)) {
                throw new ActionsParseException(String.format(
                        "Unexpected action item %s '%s' in action with id '%s'",
                        ActionsConstants.ACTION_ITEM_TYPE_KEY, gesture.type, item.id));
            }
    ...
    private void applyKeyActionToEventsMapping(final W3CItemModel item) {
        ...
        for (final W3CGestureModel gesture : gestures) {
            switch (gesture.type) {
                case ActionsConstants.ACTION_ITEM_TYPE_PAUSE:
                    timeDelta += alignDuration(extractDuration(item, gesture));
                    recordEventParams(timeDelta, null);
                    break;
                case ActionsConstants.ACTION_ITEM_TYPE_KEY_DOWN:
                    chainEntryPointDelta = timeDelta;
                case ActionsConstants.ACTION_ITEM_TYPE_KEY_UP:
                    if (gesture.value == null) {
                        throw new ActionsParseException(String.format(
                                "Missing %s key for action item '%s' of action with id '%s'",
                                ActionsConstants.ACTION_ITEM_VALUE_KEY, gesture, item.id));
                    }
                    ...
                    final KeyInputEventParams evtParams = new KeyInputEventParams(
                            ...
                            gesture.type.equals(ActionsConstants.ACTION_ITEM_TYPE_KEY_DOWN)
        ...
        if (toolType == MotionEvent.TOOL_TYPE_MOUSE && pointerIndex > 0) {
            throw new ActionsParseException(
                    String.format("No more that one simultaneous pointer is supported for %s %s",
                            ActionsConstants.PARAMETERS_KEY_POINTER_TYPE, ActionsConstants.POINTER_TYPE_MOUSE));
        }
        if (toolType == MotionEvent.TOOL_TYPE_STYLUS && pointerIndex > 0) {
            throw new ActionsParseException(
                    String.format("No more that one simultaneous pointer is supported for %s %s",
                            ActionsConstants.PARAMETERS_KEY_POINTER_TYPE, ActionsConstants.POINTER_TYPE_PEN));
        }
    }
    ...
    private void applyPointerActionToEventsMapping(final W3CItemModel item, final int pointerIndex) {
        ...
        for (int actionItemIdx = 0; actionItemIdx < gestures.size(); actionItemIdx++) {
            ...
            switch (gesture.type) {
                ...
                break;
                case ActionsConstants.ACTION_ITEM_TYPE_POINTER_DOWN: {
                ...
                break;
                case ActionsConstants.ACTION_ITEM_TYPE_POINTER_UP: {
                    if (!isPointerDown) {
                        throw new ActionsParseException(String.format(
                                "You cannot perform '%s' action without performing '%s' first at " +
                                        "%sms in '%s' chain", gesture.type, ActionsConstants.ACTION_ITEM_TYPE_POINTER_DOWN, timeDelta, actionId));
                    }
                ...
                break;
                case ActionsConstants.ACTION_ITEM_TYPE_POINTER_MOVE: {
                    long duration = alignDuration(extractDuration(item, gesture));
                    if (duration < ActionsConstants.EVENT_INJECTION_DELAY_MS) {
                        break;
                    }
                    if (actionItemIdx == 0) {
                    ...
                    final long firstActionDelta = recentDownDelta == startDelta || recentUpDelta == startDelta
                            ? startDelta + ActionsConstants.EVENT_INJECTION_DELAY_MS : startDelta;
                    long stepsCount = (startDelta + duration - firstActionDelta) / ActionsConstants.EVENT_INJECTION_DELAY_MS;
                    if (actionCode == MotionEvent.ACTION_HOVER_MOVE && !isHovering) {
                        ...
                        if (stepsCount < 3) {
                            ...
                            stepsCount = 3;
                            duration = ActionsConstants.EVENT_INJECTION_DELAY_MS * 3;
                        }
                    } else {
                    ...
                    timeDelta = firstActionDelta + ActionsConstants.EVENT_INJECTION_DELAY_MS;
                    ...
                    for (long step = 2; step <= stepsCount; step++) {
                        ...
                        timeDelta += ActionsConstants.EVENT_INJECTION_DELAY_MS;
                    }
                ...
                default:
                    throw new ActionsParseException(String.format(
                            "Unexpected action item %s '%s' in action with id '%s'",
                            ActionsConstants.ACTION_ITEM_TYPE_KEY, gesture.type, actionId));
            }
        }
    }
    ...
    public ActionTokens tokenize(List<W3CItemModel> preprocessedItems) {
        ...
        final List<W3CItemModel> emptyActions = filterActionsByType(preprocessedItems, ActionsConstants.ACTION_TYPE_NONE);
        for (final W3CItemModel emptyAction : emptyActions) {
            applyEmptyActionToEventsMapping(emptyAction);
        }

        final List<W3CItemModel> keyInputActions = filterActionsByType(preprocessedItems, ActionsConstants.ACTION_TYPE_KEY);
        for (final W3CItemModel keyAction : keyInputActions) {
            applyKeyActionToEventsMapping(keyAction);
        }

        final List<W3CItemModel> pointerActions = filterActionsByType(preprocessedItems, ActionsConstants.ACTION_TYPE_POINTER);
