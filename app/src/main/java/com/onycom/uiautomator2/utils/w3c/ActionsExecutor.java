...
public class ActionsExecutor {
    ...
    private static void logEvent(Object event, long eventTime, boolean result) {
        ...
        if (currentTime > eventTime + ActionsConstants.EVENT_INJECTION_DELAY_MS) {
    ...
    private boolean executeMotionEvents(List<MotionInputEventParams> events, long startTimestamp, Set<Integer> depressedMetaKeys) {
        ...
        for (final MotionInputEventParams event : ActionHelpers.normalizeSequence(events)) {
    ...
    public boolean execute() {
        ...
        for (long currentDelta = 0; currentDelta <= maxDelta; currentDelta += ActionsConstants.EVENT_INJECTION_DELAY_MS) {
            final List<InputEventParams> events = actionTokens.eventsAt(currentDelta);
            if (events == null || events.isEmpty()) {
                sleepTillNextEvent(startTimestamp + currentDelta + ActionsConstants.EVENT_INJECTION_DELAY_MS);
                continue;
            }
