...
import com.onycom.uiautomator2.common.exceptions.NoAlertOpenException;
...
public class AlertHelpers {
    ...
    private static AlertType getAlertType() {
        ...
        if (!Device.getUiDevice().findObjects(By.res(regularAlertTitleResIdPattern)).isEmpty()) {
            Log.d(TAG, "Regular alert has been detected");
            return AlertType.REGULAR;
        }
        if (!Device.getUiDevice().findObjects(By.res(permissionAlertTitleResIdPattern)).isEmpty()) {
    ...
    private static UiObject2 getRegularAlertButton(AlertAction action, @Nullable String buttonLabel) {
        ...
        for (final UiObject2 button : Device.getUiDevice().findObjects(By.res(regularAlertButtonResIdPattern))) {
    ...
    private static UiObject2 getPermissionAlertButton(AlertAction action, @Nullable String buttonLabel) {
        final List<UiObject2> buttons = Device.getUiDevice().findObjects(By.res(permissionAlertButtonResIdPattern));
        if (buttons.isEmpty()) {
            return null;
        }
    ...
    public static String getText() {
        ...
        final List<UiObject2> alertRoots = Device.getUiDevice().findObjects(By.res(alertContentResId));
