...
import com.onycom.uiautomator2.common.exceptions.NoSuchAttributeException;
...
import com.onycom.uiautomator2.core.UiObjectChildGenerator;
...
import com.onycom.uiautomator2.model.UiObject2Element;

import static android.view.accessibility.AccessibilityNodeInfo.AccessibilityAction.ACTION_SET_PROGRESS;
import static com.onycom.uiautomator2.model.internal.CustomUiDevice.getInstance;
import static com.onycom.uiautomator2.utils.Device.getAndroidElement;
import static com.onycom.uiautomator2.utils.ReflectionUtils.getField;
...
public abstract class ElementHelpers {
    ...
    public static List<Object> dedupe(List<Object> elements) {
        try {
            findAccessibilityNodeInfo = ReflectionUtils.method(UiObject.class, "findAccessibilityNodeInfo", long.class);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static boolean setText(final Object element, @Nullable final String text) {
        ...
        String textToSend = StringHelpers.toNonNullString(text);
        AccessibilityNodeInfo nodeInfo = AccessibilityNodeInfoGetter.fromUiObject(element);
        if (nodeInfo == null) {
            throw new ElementNotFoundException();
        }
    ...
    public static AndroidElement findElement(final BySelector ui2BySelector) throws UiAutomator2Exception {
        ...
        return Device.getAndroidElement(UUID.randomUUID().toString(), ui2Object, true);
    }
    ...
    public static String getText(Object element, boolean replaceNull) {
        if (element instanceof UiObject2) {
             ...
             */
            AccessibilityNodeInfo nodeInfo = (AccessibilityNodeInfo) ReflectionUtils.getField(UiObject2.class,
                    "mCachedNode", element);
            if (nodeInfo != null && Objects.equals(nodeInfo.getClassName(), Toast.class.getName())) {
                return StringHelpers.charSequenceToString(nodeInfo.getText(), replaceNull);
            }
        }
    ...
    private static int getScrollableOffset(AndroidElement uiScrollable) {
        ...
        if (lastScrollData == null) {
            ...
            Device.getUiDevice().swipe(x2, y2, x1, y1, MINI_SWIPE_STEPS);
        }
    ...
    private static int getTouchPadding(AndroidElement element) throws UiObjectNotFoundException, ReflectiveOperationException {
        final UiObject2 uiObject2 = element instanceof UiObject2Element
                ? Device.getUiDevice().findObject(By.clazz(((UiObject2) element.getUiObject()).getClassName()))
                : Device.getUiDevice().findObject(By.clazz(((UiObject) element.getUiObject()).getClassName()));
    ...
    private static boolean swipe(final int startX, final int startY, final int endX, final int endY) {
        ...
        return EventRegister.runAndRegisterScrollEvents(new ReturningRunnable<Boolean>() {
            @Override
            public void run() {
                setResult(Device.getUiDevice().swipe(startX, startY, endX, endY, MINI_SWIPE_STEPS));
            }
        });
    }
